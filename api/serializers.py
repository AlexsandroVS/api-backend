...
from .models import (
    ...
    Categoria, Producto, Medicamento, Factura, Venta, Pedido, DetallePedido
)
...
class MedicamentoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Medicamento
        fields = ['producto', 'tipo_presentacion', 'receta_obligatoria']

    def create(self, validated_data):
        producto_id = validated_data['producto']
        
        # Verificar si ya existe un Medicamento para este Producto
        if Medicamento.objects.filter(producto=producto_id).exists():
            raise serializers.ValidationError("Ya existe un medicamento asociado a este producto.")
